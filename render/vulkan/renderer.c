...
static struct wlr_buffer *vk_allocator_create_buffer(struct wlr_allocator *wlr_alloc,
	...
	const struct wlr_vk_format_modifier_props *mod_props =
		vulkan_format_props_find_modifier(alloc->dev->format_props, img_mod_props.drmFormatModifier, true);
...
static struct wlr_allocator *vulkan_get_allocator(struct wlr_renderer *wlr_renderer) {
	struct wlr_vk_renderer *renderer = vulkan_get_renderer(wlr_renderer);
