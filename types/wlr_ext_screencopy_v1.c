...
#define SCREENCOPY_MANAGER_V1_VERSION 1

struct wlr_ext_screencopy_session_v1 {
	struct wl_resource *resource;
	struct wlr_ext_image_source_v1 *source;
...
struct wlr_ext_screencopy_cursor_session_v1 {
	struct wl_resource *resource;
	struct wlr_ext_image_source_v1_cursor *source;
	bool screencopy_session_created;
...
static const struct ext_screencopy_frame_v1_interface frame_impl;
static const struct ext_screencopy_session_v1_interface session_impl;
static const struct ext_screencopy_cursor_session_v1_interface cursor_session_impl;

static struct wlr_ext_screencopy_frame_v1 *frame_from_resource(
		struct wl_resource *resource) {
	assert(wl_resource_instance_of(resource,
		&ext_screencopy_frame_v1_interface, &frame_impl));
...
static struct wlr_ext_screencopy_cursor_session_v1 *cursor_session_from_resource(
		struct wl_resource *resource) {
	assert(wl_resource_instance_of(resource,
		&ext_screencopy_cursor_session_v1_interface, &cursor_session_impl));
	return wl_resource_get_user_data(resource);
}

static void frame_destroy(struct wlr_ext_screencopy_frame_v1 *frame) {
	if (frame == NULL) {
		return;
	}
...
void wlr_ext_screencopy_frame_v1_ready(struct wlr_ext_screencopy_frame_v1 *frame,
	...
	for (int i = 0; i < rects_len; i++) {
		...
		ext_screencopy_frame_v1_send_damage(frame->resource,
			rect->x1, rect->y1, rect->x2 - rect->x1, rect->y2 - rect->y1);
	}

	ext_screencopy_frame_v1_send_transform(frame->resource, transform);
	ext_screencopy_frame_v1_send_presentation_time(frame->resource,
		presentation_time->tv_sec >> 32, presentation_time->tv_sec,
		presentation_time->tv_nsec);
	ext_screencopy_frame_v1_send_ready(frame->resource);
	frame_destroy(frame);
}
...
static void frame_handle_attach_buffer(struct wl_client *client,
	...
	struct wlr_ext_screencopy_frame_v1 *frame = frame_from_resource(frame_resource);
	if (frame == NULL) {
		return;
	}
	...
	if (frame->capturing) {
		wl_resource_post_error(frame->resource,
			EXT_SCREENCOPY_FRAME_V1_ERROR_ALREADY_CAPTURED,
...
static void frame_handle_damage_buffer(struct wl_client *client,
	...
	if (x < 0 || y < 0 || width <= 0 || height <= 0) {
		wl_resource_post_error(frame->resource,
			EXT_SCREENCOPY_FRAME_V1_ERROR_INVALID_BUFFER_DAMAGE,
...
static void frame_handle_capture(struct wl_client *client,
	...
	if (frame->buffer == NULL) {
		wl_resource_post_error(frame->resource,
			EXT_SCREENCOPY_FRAME_V1_ERROR_NO_BUFFER,
			"No buffer attached");
		return;
	}
	...
	struct wlr_ext_image_source_v1 *source = frame->session->source;
...
static void session_handle_create_frame(struct wl_client *client,
	...
	struct wlr_ext_screencopy_session_v1 *session = session_from_resource(session_resource);

	if (session->frame != NULL) {
		wl_resource_post_error(session_resource,
			EXT_SCREENCOPY_SESSION_V1_ERROR_DUPLICATE_FRAME,
	...
	struct wl_resource *frame_resource = wl_resource_create(client,
		&ext_screencopy_frame_v1_interface, version, new_id);
	...
	if (session == NULL) {
		ext_screencopy_frame_v1_send_failed(frame_resource,
			EXT_SCREENCOPY_FRAME_V1_FAILURE_REASON_STOPPED);
		return;
	}

	struct wlr_ext_screencopy_frame_v1 *frame = calloc(1, sizeof(*frame));
	if (frame == NULL) {
...
static void session_send_constraints(struct wlr_ext_screencopy_session_v1 *session) {
	struct wlr_ext_image_source_v1 *source = session->source;

	ext_screencopy_session_v1_send_buffer_size(session->resource,
		source->width, source->height);

	for (size_t i = 0; i < source->shm_formats_len; i++) {
		ext_screencopy_session_v1_send_shm_format(session->resource,
			source->shm_formats[i]);
	}
	...
	if (source->dmabuf_formats.len > 0) {
		...
		ext_screencopy_session_v1_send_dmabuf_device(session->resource,
			&dev_id_array);
	}
	for (size_t i = 0; i < source->dmabuf_formats.len; i++) {
		...
		ext_screencopy_session_v1_send_dmabuf_format(session->resource,
			fmt->format, &modifiers_array);
	}

	ext_screencopy_session_v1_send_done(session->resource);
}
...
static void session_destroy(struct wlr_ext_screencopy_session_v1 *session) {
	...
	if (session->frame != NULL) {
		wlr_ext_screencopy_frame_v1_fail(session->frame,
			EXT_SCREENCOPY_FRAME_V1_FAILURE_REASON_STOPPED);
	}

	ext_screencopy_session_v1_send_stopped(session->resource);
...
static void session_handle_source_destroy(struct wl_listener *listener, void *data) {
	struct wlr_ext_screencopy_session_v1 *session = wl_container_of(listener, session, source_destroy);
...
static void session_handle_source_frame(struct wl_listener *listener, void *data) {
	struct wlr_ext_screencopy_session_v1 *session = wl_container_of(listener, session, source_frame);
	struct wlr_ext_image_source_v1_frame_event *event = data;

	pixman_region32_union(&session->damage, &session->damage, event->damage);

	struct wlr_ext_screencopy_frame_v1 *frame = session->frame;
	if (frame != NULL && frame->capturing &&
		...
		// TODO: don't clear on copy error
		pixman_region32_clear(&session->damage);
	}
}
...
static void session_create(struct wl_resource *parent_resource, uint32_t new_id,
		struct wlr_ext_image_source_v1 *source, uint32_t options) {
	...
	struct wl_resource *session_resource = wl_resource_create(client,
		&ext_screencopy_session_v1_interface, version, new_id);
	...
	if (source == NULL) {
		ext_screencopy_session_v1_send_stopped(session_resource);
		return;
	}

	struct wlr_ext_screencopy_session_v1 *session = calloc(1, sizeof(*session));
	if (session == NULL) {
		wl_resource_post_no_memory(parent_resource);
		return;
	}
...
static void cursor_session_destroy(struct wlr_ext_screencopy_cursor_session_v1 *cursor_session) {
	if (cursor_session == NULL) {
		return;
	}
	if (cursor_session->source->entered) {
		ext_screencopy_cursor_session_v1_send_leave(cursor_session->resource);
	}
...
static void cursor_session_handle_get_screencopy_session(struct wl_client *client,
		struct wl_resource *cursor_session_resource, uint32_t new_id) {
	struct wlr_ext_screencopy_cursor_session_v1 *cursor_session =
		cursor_session_from_resource(cursor_session_resource);

	if (cursor_session != NULL && cursor_session->screencopy_session_created) {
		wl_resource_post_error(cursor_session_resource,
			EXT_SCREENCOPY_CURSOR_SESSION_V1_ERROR_DUPLICATE_SESSION,
			"get_screencopy_session sent twice");
		return;
	}

	struct wlr_ext_image_source_v1 *source = NULL;
	if (cursor_session != NULL) {
...
static const struct ext_screencopy_cursor_session_v1_interface cursor_session_impl = {
	...
	.get_screencopy_session = cursor_session_handle_get_screencopy_session,
};
...
static void cursor_session_update(
		struct wlr_ext_screencopy_cursor_session_v1 *cursor_session) {
	struct wlr_ext_image_source_v1_cursor *cursor_source = cursor_session->source;

	if (cursor_source->entered && !cursor_session->prev.entered) {
		ext_screencopy_cursor_session_v1_send_enter(cursor_session->resource);
	}
	if (!cursor_source->entered && cursor_session->prev.entered) {
		ext_screencopy_cursor_session_v1_send_leave(cursor_session->resource);
	}
	...
	if (cursor_source->x != cursor_session->prev.x ||
		...
		ext_screencopy_cursor_session_v1_send_position(cursor_session->resource,
	...
	if (cursor_source->hotspot.x != cursor_session->prev.hotspot.x ||
		...
		ext_screencopy_cursor_session_v1_send_hotspot(cursor_session->resource,
...
static void manager_handle_create_session(struct wl_client *client,
	...
	struct wlr_ext_image_source_v1 *source =
		wlr_ext_image_source_v1_from_resource(source_resource);
...
static void manager_handle_create_pointer_cursor_session(struct wl_client *client,
		...
		uint32_t options) {
	struct wlr_ext_image_source_v1 *source = wlr_ext_image_source_v1_from_resource(source_resource);
	...
	struct wlr_ext_image_source_v1_cursor *source_cursor = NULL;
	...
	struct wl_resource *cursor_session_resource = wl_resource_create(client,
		&ext_screencopy_cursor_session_v1_interface, version, new_id);
	...
	struct wlr_ext_screencopy_cursor_session_v1 *cursor_session = calloc(1, sizeof(*cursor_session));
	if (cursor_session == NULL) {
		wl_resource_post_no_memory(manager_resource);
		return;
	}
...
struct wlr_ext_screencopy_manager_v1 *wlr_ext_screencopy_manager_v1_create(
	...
	assert(version <= SCREENCOPY_MANAGER_V1_VERSION);

	struct wlr_ext_screencopy_manager_v1 *manager = calloc(1, sizeof(*manager));
	if (manager == NULL) {
		return NULL;
	}

	manager->global = wl_global_create(display,
		&ext_screencopy_manager_v1_interface, version, manager, manager_bind);
