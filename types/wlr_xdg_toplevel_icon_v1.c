...
static void icon_handle_resource_destroy(struct wl_resource *resource) {
	wlr_xdg_toplevel_icon_v1_unlock(icon_from_resource(resource));
}
...
static void manager_handle_create_icon(struct wl_client *client, struct wl_resource *resource,
	...
	struct wl_resource *icon_resource = wl_resource_create(client,
		&xdg_toplevel_icon_manager_v1_interface, wl_resource_get_version(resource), id);
	if (resource == NULL) {
		wl_client_post_no_memory(client);
		free(icon);
		return;
	...
	wl_resource_set_implementation(resource, &icon_resource, icon, icon_handle_resource_destroy);

	wlr_xdg_toplevel_icon_v1_lock(icon);
}
