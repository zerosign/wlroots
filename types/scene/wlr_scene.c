...
static void scene_node_opaque_region(struct wlr_scene_node *node, int x, int y,
	...
	if (node->type == WLR_SCENE_NODE_RECT) {
		...
		if (!scene_buffer->buffer || !buffer_is_opaque(scene_buffer->buffer)) {
...
static void scene_buffer_handle_release(struct wl_listener *listener, void *data) {
...
void wlr_scene_buffer_set_buffer_with_damage(struct wlr_scene_buffer *scene_buffer,
	...
	if (buffer) {
		...
		scene_buffer->buffer_locked = true;
		scene_buffer->buffer_release.notify = scene_buffer_handle_release;
...
static void scene_node_get_size(struct wlr_scene_node *node,
	...
	*height = 0;
	...
	case WLR_SCENE_NODE_BUFFER:;
		...
		} else if (scene_buffer->buffer || scene_buffer->texture) {
			int w, h;
			if (scene_buffer->buffer) {
				w = scene_buffer->buffer->width;
				h = scene_buffer->buffer->height;
			} else {
				w = scene_buffer->texture->width;
				h = scene_buffer->texture->height;
			}
...
static bool scene_buffer_can_consider_direct_scanout(struct wlr_scene_buffer *buffer,
	...
	if (node->type != WLR_SCENE_NODE_BUFFER || buffer->buffer == NULL) {
		return false;
	}
